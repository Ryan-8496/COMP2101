...
routerHostname=$(route | grep -w default | awk '{print $2}')
networkNumber=$(ip route | grep -w via | awk '{print $3}' | cut -f1-3 -d.)"."$(((`ip route | grep -w via | awk '{print $3}' | cut -f4 -d.` - 1)))
networkName=
externalIP=$(curl -s icanhazip.com)
externaName=$(getent hosts $(curl -s icanhazip.com) | awk '{print $2}')
